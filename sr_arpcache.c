    ...
    if(difftime(cur_time, arp_req->sent) >= 1.0) {
        if(arp_req->times_sent >=5 ) {
            ...
            while(packet != NULL) {
                ...
                sr_ethernet_hdr_t *new_eth_hdr = (sr_ethernet_hdr_t *)icmp;
                construct_ETHERNET_header(eth_hdr, new_eth_hdr, lpm_if);                        
                
                /* Construct the IP header */
                sr_ip_hdr_t *new_ip_hdr = (sr_ip_hdr_t *)((unsigned char *)icmp + sizeof(sr_ethernet_hdr_t));
                construct_IP_header(ip_hdr, new_ip_hdr, packet_if);    
                
                /* Construct the ICMP header */
                sr_icmp_t3_hdr_t *icmp_hdr = (sr_icmp_t3_hdr_t *)((unsigned char *)icmp + sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t));
                construct_ICMP_header(ip_hdr, icmp_hdr, 3, 1);                    
                
                struct sr_arpentry *entry = sr_arpcache_lookup(cache, ip_hdr->ip_src);

                if(entry) {
                    sr_send_packet(sr, icmp, sizeof(sr_ethernet_hdr_t) + sizeof(sr_ip_hdr_t) + sizeof(sr_icmp_t3_hdr_t), lpm_if->name);
                    free(icmp);
                } else {
                    struct sr_arpreq *arp_queue_req = sr_arpcache_queuereq(cache, ip_hdr->ip_src, icmp, 
...
void sr_broadcast_arpreq(struct sr_instance* sr, struct sr_arpreq *req) {
    ...
    sr_arp_hdr_t *arp_hdr = (sr_arp_hdr_t *)((char *)arp_packet + sizeof(sr_ethernet_hdr_t));
    ...
    sr_send_packet(sr, arp_packet, sizeof(sr_ethernet_hdr_t)+sizeof(sr_arp_hdr_t), broad_if->name);  
    free(arp_packet);
    return;
  }
